...
assert.equal(statSync(resolve(root, 'course-fee-schedule.jpg')).size, 837964, 'Application pricing image should use the newly supplied October schedule banner');
assert.ok(html.includes('alt="10월 온라인반과 서울 오프라인반 강의 일정 및 얼리버드 수강료 안내"'), 'Application pricing image alt text should describe the updated October schedule');
