...
- Vercel CLI 인증이 없고 연결 앱에서 팀만 조회되며 프로젝트가 404이면, 새 프로젝트를 만들지 말고 기존 운영 계정 로그인을 복구한 뒤 배포한다.

- 내장 브라우저에서 YouTube iframe이 검게 보이더라도 일반 브라우저에서 실제 재생 상태를 별도 검증한다. iframe 생성만으로 재생 성공을 판단하지 않는다.
